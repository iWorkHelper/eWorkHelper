...
当前版本为 `1.3.0`。版本与发布规则见 [`docs/RELEASE.md`](docs/RELEASE.md)。

## 主要功能

- 在 Excel Ribbon 的自定义“工作助手”选项卡中提供“批量过滤”入口。
- 在需要作为基准的 Excel 工作簿中点击“Excel 差异对比”，然后选择待比较工作簿；可配置 Sheet 映射、差异高亮和基准值备注并生成结果副本。比较期间显示独立累计耗时、阶段和 Sheet 进度，结果参与比较的 Sheet 会在 A 列显示按相同行号计算的 ADD/DEL/CHG 状态。两个工作簿须先保存所有修改。
...
差异对比使用方法：先激活并保存要作为基准的工作簿，在“工作助手”→“数据工具”点击“Excel 差异对比”，再选择待比较工作簿、添加 Sheet 映射并设置结果路径。窗口顶部仅显示固定的基准工作簿文件名；如需更换，请关闭窗口、激活新的基准工作簿后重新进入。“基准内容写入批注”默认勾选，可按需关闭。比较完成后可从窗口再次点击“打开结果”；阶段、差异数量和详细状态显示在底部。未保存的新工作簿或有未保存修改的工作簿不会被静默按旧磁盘内容比较。
