...
- `MainRibbon.*`：唯一的 Ribbon Designer 实现，提供 `eWorkHelper` Tab、正式功能入口以及末端“关于”入口；关于信息从程序集元数据动态读取版本。
- `BatchFilterForm.cs`：批量过滤交互窗口，负责条件输入、模式选择、提示和结果展示。
- `BatchFilterService.cs`：负责 Excel 上下文识别、筛选区域选择、条件标准化、匹配计算与原生 AutoFilter 操作。
- `UnmergeAndFillService.cs`：读取当前 Excel Selection，发现并去重 MergeArea，使用原生 UnMerge 后批量回填值或公式。
...
取消合并并填充从 `Application.Selection` 获取 Excel Range，遍历全部 `Areas`，仅处理与选区相交的完整 `MergeArea`。服务在取消合并前读取左上角的 `Value2` 或 `FormulaR1C1`，调用 Excel 原生 `Range.UnMerge()`，再把内容一次性写入原区域；不改变工作表结构，也不主动复制或重构格式。
...
- 取消合并扫描先按范围读取 `MergeCells` 并递归拆分混合块；无合并块一次跳过，发现的 MergeArea 按工作表与外部绝对地址去重，避免大范围中的无意义逐单元格 COM 调用和重复 UnMerge。
